...
int InputArray(const char *sf, int ***a , size_t *m, size_t **n)
{
 ...
 *n=(size_t*)malloc((*m)*sizeof(size_t)); 
 ...
 if(f)
 {
  for(i=0; wfgets(&str, &l, f)!=NULL; i++)
  {
   for(I=0; I<2; I++)
   {
    for((*n)[i]=0, s=str; sscanf(s,"%d%n",(I==0?&tmp:(*a)[i]+(*n)[i]),&sz)==1; s+=sz,(*n)[i]++);
    if(I==0) (*a)[i]=((*n)[i]==0?NULL:(int*)malloc((*n)[i]*sizeof(int)));
   }
  }
  fclose(f);
  while((*m)>0 && (*n)[(*m)-1]==0)(*m)--;

...
int main(int argc,char *argv[])
{
  ...
  task=1;
  if(!((argc==8 || argc==9) && sscanf(argv[1],"%d",&i)==1 && i>0 &&sscanf(argv[2],"%d",&j)==1 
  && j>0 && sscanf(argv[3],"%lf",&c)==1  && sscanf(argv[4],"%d",&n)==1 && n>0 &&
  sscanf(argv[5],"%d",&m)==1 && m>0 && 
  sscanf(argv[6],"%d",&p)==1 && p>=0 && 
  sscanf(argv[7],"%d",&k)==1 && k>=0 && i<=m && j <= m))
  {
